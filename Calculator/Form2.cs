    ...
    public partial class Form2 : Form
    {
        ...
        private void buttonNumber_Click(object sender, EventArgs e)
        {
            ...
            if (isNewEntry)
            {
                ...
                // overwrite with a comma, but place a 0 in front of it. If a special number is
                //pressed, overwrite it with that value and set isNewEntry back to true
                //to prevent further concatenation to that spacial number. Otherwise
                //overwrite it with just the number
                switch (b.Text)
                {
                    case ",":
                        textBox1.Text = "0,";
                        break;
                    case "π":
                        textBox1.Text = Math.PI.ToString(CultureInfo.GetCultureInfo("nl-NL").NumberFormat);
                        isNewEntry = true;
                        break;
                    case "e":
                        textBox1.Text = Math.E.ToString(CultureInfo.GetCultureInfo("nl-NL").NumberFormat);
                        isNewEntry = true;
                        break;
                    default:
            ...
            {
                //special numbers can only be entered as new entry
                if (b.Text != "π" && b.Text != "e")
                    textBox1.Text += b.Text;
            }
        ...
        {
            Button b = (Button)sender;
            //For normal operations: if an operation is not yet chosen:
            ...
            if (!operatorPressed)
            {
                ...
                else
                {
                    ...
                    if (!resultPressed)
                    {
                        //this code is executed if buttonResult was not previously pressed
                    ...
                    {
                        //this code is executed if it was
                        resultPressed = false;
                    }
        ...
        {
            //Calculates the result based on the previously chosen operation and parsed
            //numbers.
